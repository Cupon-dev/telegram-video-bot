// Ultimate Telegram Bot with HTML Image Gateway
...
// Generate HTML page with clickable image
function generateHTMLPage(imageUrl, videoUrl) {
    return `
<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class="container">
        <div style="position: relative; display: inline-block;">
            <img src="${imageUrl}" class="clickable-image" onclick="openVideo()" alt="Video thumbnail">
            <div class="play-button" onclick="openVideo()">
                <div class="play-icon"></div>
            </div>
        </div>
        <p class="instruction">Tap the image to start watching ▶️</p>
    </div>

    <script>
        function openVideo() {
            // Open video in Telegram's built-in browser
            window.open('${videoUrl}', '_blank');
            
            // Optional: Close the web app after opening video
            setTimeout(() => {
        ...
        // Open video automatically after 3 seconds
    ...
    if (session && session.step === 'waiting_video') {
        if (text.includes('mediadelivery.net') || text.includes('iframe')) {
            try {
                ...
                // Get direct image URL for HTML page
                const fileLink = await bot.getFileLink(session.imageFileId);
                const imageUrl = fileLink;
                
                // Generate HTML page with clickable image
                const htmlContent = generateHTMLPage(imageUrl, hiddenVideoLink);
                
                // For now, use regular web_app with the video URL
                // In production, you'd host the HTML page and use its URL
...
async function postToChannel(channelId, session, originalMessage) {
    try {
        ...
        // Generate HTML page with clickable image
        const htmlContent = generateHTMLPage(imageUrl, session.hiddenVideoLink);
        
        // For now, use web_app with fallback
        try {
